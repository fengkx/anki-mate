...
当前实现中，当满足以下条件时，进入词条预览后会自动尝试一次生成：

- 当前词条已完成 lookup
- 当前方言尚无已保存的 `stress syllables`
- 本地 LLM server 已运行，且当前不处于需要手动引导用户配置模型的状态
- 当前没有进行中的同词条同方言生成任务

自动生成只尝试一次，不做页面内无限重试；若失败，保留轻量手动刷新入口。
