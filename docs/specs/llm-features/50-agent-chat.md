...
一期不引入原生 function calling,tool 调用用"文本回复 + 尾部 JSON fence"的格式做后处理解析,避免本地 llama 在原生 tool calling 上的不稳定。
...
当前状态：Phase 2b 的 chat-template bridge 统一化已完成。`LLMService.generate(...)` 已成为单一生成入口，`tools` / `toolChoice` / `parallelToolCalls` 走统一 RPC 与 server 推理路径；`InferenceEngine.loadModel(...)` 会在加载期 eager 初始化 llama.cpp chat template handle，避免首次生成时才暴露模板不可用错误。
...
- bridge 统一化：完成。prompt / grammar / output parse 全部通过 llama.cpp chat template bridge，普通 chat、structured output、tool calling 共享同一生成管线
- 测试：
...
- `Sources/DictKitApp/Views/Agent/`
  - `AgentChatView.swift`
  - `AgentContextBar.swift`
  - `AgentMessageList.swift` / `AgentMessageBubble.swift` / `AgentToolTraceRow.swift`
  - `AgentActionCard.swift`
  - `AgentComposer.swift`
- 在 `AIContentView` 顶部加 `Structured` / `Chat` 分段控件
- `CardPreviewView` 订阅 `AgentSession.previewOverrideArtifacts`
- 测试：view model 层测试走 `AgentSession`,view 快照仅覆盖关键组件
