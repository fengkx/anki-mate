...
- `Recall Card` 的默认目标是：
  - 看到中文提示
  - 回忆英文单词
- `targeted letter cloze` 只作为 Recall Card 的一个变体存在，不是独立卡型
...
- `Recall Card` 继续作为一级入口显示，但在交互语义上视为单张反向卡编辑器
...
- `Recall Card` 顶层始终可见，但展开后进入单卡编辑形态
...
- 生成 recall 导向卡片草稿
- 支持 `Chinese cue -> English word` 的反向回忆卡
- 支持 `targeted letter cloze` 作为变体
- 产物可直接导出为卡片内容
