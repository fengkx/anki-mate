...
final class LLMPromptTests: XCTestCase {
    ...
    func testRecallDecisionPromptIncludesLearningAidsAllowedModesAndFrontBackContract() {
        ...
        XCTAssertTrue(prompt.user.contains("choose the gap position yourself"))
        XCTAssertTrue(prompt.user.contains("<examples_do_not_copy>"))
        XCTAssertTrue(prompt.user.contains("<example mode=\"targeted_letter_cloze\">"))
        XCTAssertTrue(prompt.user.contains("<bad_front>"))
        XCTAssertTrue(prompt.user.contains("Never copy XML tags, labels, or example metadata into draft.front"))
        XCTAssertFalse(prompt.user.contains("front: 收到 · 拼出完整英文单词 | back: receive"))
